...
const InnerCircle = () => {
  // Always set Partner as the default tab
  ...
  // Load groups and shared entries when tab changes
  useEffect(() => {
    const fetchGroups = () => {
      // Get only the valid groups using our helper function
      ...
      const formattedEntries = entries.map(entry => {
        ...
        return {
          ...
          comments: comments.map(comment => ({
            ...
            groupId: groupId // Add groupId to comments to determine if from Private group
          })),
  ...
  const handleInviteMember = async () => {
    ...
    try {
      ...
      if (!emailSent) {
        // Member was added but email failed - we could handle this differently
        toast({
          title: "Member added",
  ...
  const handleLinkPartner = async () => {
    ...
    try {
      ...
      if (sent) {
        ...
        // Also add them to the partner group
  ...
  // Special handling for Partner tab
  const isPartnerTab = selectedTab === "partner";
  // Special handling for Private tab
                        ...
                        {(group.members || []).map((member, index) => (
                          <div key={index} className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                {isPrivateTab ? (
                                  // For Private tab, show anonymized names to owner
                ...
                {sharedEntries.length > 0 ? (
                  sharedEntries.map(entry => (
                    <GroupEntryCard 
                      ...
                      entry={{
                        ...entry,
                        // Special handling for Private group anonymization
